...
import { Switch, Route } from "react-router-dom"; //react-router-dom페이지
...
function App() {
  return (
    <div>
      <Switch>
        <Route exact={true} path="/"> {/* 주소 뒤에 아무것도 없을 때 */}
          <MainPageComponent />
        </Route>
        <Route exact={true} path="/products/:id"> {/* 주소 뒤에 "/product가 있을때"
                                                      :뒤에 자유롭게 숫자 입력 가능 ex)./product/2*/}
          <ProductPage />
        </Route>
        <Route exact={true} path="/upload"> {/* 주소 뒤에 "/upload가 있을때" */}
          <UploadPage />
        </Route>
      </Switch>
    </div>
